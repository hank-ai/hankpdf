...
- **Native text-layer preservation** — when `--ocr` is on AND the input PDF has an existing text layer, hankpdf now reads it via pdfium and reuses the original text + bounding boxes verbatim instead of running Tesseract. The output is byte-faithful to the source's text, with no Tesseract recognition errors and no per-page Tesseract subprocess cost on the inputs that need it most (already-OCR'd archives). Falls back to Tesseract automatically on true scans (no upstream text layer). New `pdf_smasher.engine.text_layer.extract_native_word_boxes` helper; tests in `tests/unit/engine/test_native_text_extraction.py`. The `--ocr` flag's API and CLI surface are unchanged — this is a transparent quality + perf improvement.
